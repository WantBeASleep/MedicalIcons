...
DEFAULT_TEXTURES_DIR = PROJECT_ROOT / "internal" / "textures"
DEFAULT_STATUSICONS_DIR = PROJECT_ROOT / "internal" / "statusicons"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        metavar="STATUS_CSV",
        help="CSV mapping item identifier to status icon name. Default: internal/scripts/build_project/statusicons.csv.",
    )
    parser.add_argument(
        "--disable-staus-icons",
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="DIR",
        help="Save 64x64 item icons after status icon overlay. Default: internal/scripts/build_project/status_icons.",
    )
    parser.add_argument(
        "--atlas-out",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Directory with internal texture item asset folders.",
    )
    parser.add_argument(
        "--statusicons-dir",
        type=Path,
...
def atlas_rect(entry: AtlasEntry) -> str:
    return f"{{ {entry.x}, {entry.y}, {entry.width}, {entry.height} }}"


def render_atlases_lua(ctx: BuildContext, icons_path: Path, sprites_path: Path) -> str:
    lines = [
        "-- This file is generated by internal/scripts/build_project/build_project.py.",
        "-- Manual runtime constants belong in data.lua.",
        "",
        "local atlases = {",
        "    assets = {",
        f"        icons = {lua_quote(mod_path(icons_path))},",
        f"        sprites = {lua_quote(mod_path(sprites_path))},",
        "    },",
        "",
        "    items = {",
    ]
    ...
    for identifier in sorted(item_by_id):
        ...
        lines.extend(
            [
                f"        [{lua_quote(identifier)}] = {{",
                f"            texture = {lua_quote(item.texture)},",
                f"            icon = {{ rect = {atlas_rect(icon_entry)} }},",
                f"            sprite = {{ rect = {atlas_rect(sprite_entry)} }},",
                "        },",
                "",
            ]
        )

    lines.extend(["    },", "}", "", "return atlases", ""])
    return "\n".join(lines)
